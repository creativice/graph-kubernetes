...
export function createPodEntity(data: k8s.V1Pod) {
  // TODO: remove data.spec.containers array from this (from rawData)
...
// TODO: this isn't really pod, but if we're going to break out container data
// off of the pod resource, we may not want to create different folder just to house this converter, or do we?
  ...
  return createIntegrationEntity({
    entityData: {
      ...
      assign: {
        ...
        // minLength issue
        // TODO: use function for this (see if there's uid as well)
        _key: `container:${data.name}`,
        name: data.name,
